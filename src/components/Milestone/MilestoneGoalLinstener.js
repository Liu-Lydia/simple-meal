    ...
    const isNewMilestoneGoal = async () => {
        ...
        if (loginBool && milestoneListener) {
            ...
            await fetch(url, {
                ...
                .then((r) => r.json())
                .then((obj) => {
                    if (obj.id == userID) {//確認還是同一人
                        if (milestoneGoalList.length != obj.result.length)//如果成就數量不同
                        {
                            //一對一比較
                            for (let i = 0; i < obj.result.length; i++) {
                                let exist = false;//設定boolean 來檢查有沒有找到
                                for (let j = 0; j < milestoneGoalList.length; j++) {
                                    if (obj.result[i].milestone_sid == milestoneGoalList[j].milestone_sid)//比較兩者的milestone_sid 是否相同
                                    {
                                        exist = true;
                                    }
                                }
                                if (!exist) {//如果該成就不存在於原有陣列中 顯示出來
                    ...
                    } else {//取得不同UserID 取消監聽 重新取得 MilestoneGoalList
